...
(function(window) {
	...
	me.DefaultLoadingScreen = me.ScreenObject.extend({
		...
		onResetEvent : function() {
			// melonJS logo
			this.logo1 = new me.Font('Varela Round', 32, '#669ecc', 'middle');
			this.logo2 = new me.Font('Varela Round', 32, '#4a7ca6', 'middle');
			this.logo2.bold();
		...
		draw : function(context) {
			...
			// measure the logo size
			var logo1_width = this.logo1.measureText(context, "ms").width;
			var xpos = (me.video.getWidth() - logo1_width - this.logo2.measureText(context, "GELEE").width) / 2;
			var ypos = (me.video.getHeight() / 2) + (this.logo2.measureText(context, "ms").height);
			...
			this.logo1.draw(context, 'ms', xpos , ypos);
			xpos += logo1_width;
			this.logo2.draw(context, 'GELEE', xpos, ypos);
...
})(((typeof(module) !== 'undefined') && module.exports) || window);;function InputObserver(){
...
;game.RoyalJellyEntity = me.ObjectEntity.extend({
    init: function(x, y, settings) {
		this.renderable = new me.SpriteObject(
            0, 0,
            me.loader.getImage("royal-jelly-spritesheet"),
            32, 32
        );
		this.renderable.offset.set(0, 0);
		settings.image = this.renderable.image;
...
});;game.LanternEntity = me.ObjectEntity.extend({
    init: function(x, y, settings) {
	
		this.renderable = new me.SpriteObject(
            0, 0,
            me.loader.getImage("lantern-spritesheet"),
            32, 32
        );
		this.renderable.offset.set(0, 0);
		settings.image = this.renderable.image;
...
;game.WaterfallEntity = me.ObjectEntity.extend({
    init: function(x, y, settings) {
	
		this.renderable = new me.SpriteObject(
            0, 0,
            me.loader.getImage("waterfall-spritesheet"),
            32, 32
        );
		this.renderable.offset.set(0, 0);
		settings.image = this.renderable.image;
...
;game.Stage1Movie = me.ScreenObject.extend({
	init : function()
	{
      this.parent(true);
    },
	...
	onResetEvent: function(){
		game.data.frameCount = 0;
		this.images = [];
		this.images[0] = {image: me.loader.getImage("stage-1-cutscene-1"), x: 0, y: 0};
		this.images[1] = {image: me.loader.getImage("stage-1-cutscene-2"), x: 0, y: 480};
		this.images[2] = {image: me.loader.getImage("stage-1-cutscene-3"), x: 0, y: 480};
		this.images[3] = {image: me.loader.getImage("stage-1-cutscene-4"), x: 0, y: 480};
...
});;game.Stage2Movie = me.ScreenObject.extend({
	init : function()
	{
      this.parent(true);
    },
	...
	onResetEvent: function(){
		game.data.frameCount = 0;
		this.images = [];
		this.images[0] = {image: me.loader.getImage("stage-2-cutscene-1"), x: 0, y: 0};
		this.images[1] = {image: me.loader.getImage("stage-2-cutscene-2"), x: 0, y: 480};
		this.images[2] = {image: me.loader.getImage("stage-2-cutscene-3"), x: 0, y: 480};
		this.images[3] = {image: me.loader.getImage("stage-2-cutscene-4"), x: 0, y: 480};
	...
	update: function(){
		if (game.data.inputObserver.isPausePressed() || (this.imageCounter + 1 === this.images.length && game.data.inputObserver.isAcceptingInput && game.data.inputObserver.isShootPressed())){
			...
			this.isMovingFrame = true;
			me.audio.stopTrack("movie");
			me.state.change(game.level_data.STAGE2_PT1);
		} else if (!this.isMovingFrame && game.data.inputObserver.isAcceptingInput && game.data.inputObserver.isShootPressed()){
			this.isMovingFrame = true;
		} else if (this.isMovingFrame){
			this.images[this.imageCounter + 1].y = this.images[this.imageCounter + 1].y - 20;
...
;game.Stage2pt1 = me.ScreenObject.extend({
	...
	onResetEvent: function() {
		if (me.audio.isAudioEnable()){
			if (me.audio.getCurrentTrack() !== "stage-2-bg"){
				me.audio.playTrack("stage-2-bg");
			} else {
				me.audio.resumeTrack();
			}
	...
	update: function() {
		...
		if (collision && collision.obj.type == game.data.WARP_TYPE){
			me.audio.pauseTrack();
			game.data.isBossDead   = false;
			game.data.checkpoint   = game.level_data.STAGE2_PT3;
			me.state.change(game.level_data.STAGE2_PT3);
		}		
		
		return true;
	}
});
;game.Stage2pt3 = me.ScreenObject.extend({
	 // override the default constructor
	init : function()
	{
      //call the parent constructor giving true
	...
	onResetEvent: function() {
		...
		me.levelDirector.loadLevel("jungle3");
	...
	update: function() {
		...
		if (game.data.isBossDead){
			me.audio.stopTrack();
			me.state.change(game.level_data.STAGE3_MOVIE);
		}
...
});;game.Stage3pt1 = me.ScreenObject.extend({
	 // override the default constructor
	init : function()
	{
      //call the parent constructor giving true
